...
class TestEndorsementManager(IsolatedAsyncioTestCase):
    ...
    async def test_auto_endorsement_setup_as_controller_no_server_url(self):
        self.profile.settings.set_value(
            "plugin_config",
    ...
    async def test_auto_endorsement_setup_as_controller_with_previous_connection(self):
        self.profile.settings.set_value(
            "plugin_config",
    ...
    async def test_auto_endorsement_setup_as_controller_no_endorser_invitation(self):
        self.profile.settings.set_value(
            "plugin_config",
            {"did-webvh": {"role": "controller", "server_url": "http://localhost:8000"}},
        )
    ...
    async def test_auto_endorsement_setup_as_controller_bad_invitation(
    ...
    async def test_auto_endorsement_setup_as_controller_no_active_connection(self, *_):
        self.profile.settings.set_value("plugin_config.did-webvh.role", "controller")
        self.profile.settings.set_value(
            "plugin_config",
            {
    ...
    @mock.patch.object(OutOfBandManager, "receive_invitation")
    async def test_auto_endorsement_setup_as_controller_conn_becomes_active(self, *_):
        self.profile.settings.set_value("plugin_config.did-webvh.role", "controller")
        self.profile.settings.set_value(
            "plugin_config",
            {
