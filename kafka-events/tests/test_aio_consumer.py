...
import pytest
from kafka_queue.aio_consumer import AIOConsumer
...
async def test_consume_kafka_msg(mocker: MockerFixture):
    ...
    aio_kafka = mocker.patch("kafka_queue.aio_consumer.AIOKafkaConsumer").return_value
...
async def test_raised_exception_in_consume_kafka_msg(mocker: MockerFixture):
    ...
    def aux_subscribe(*args, **kwargs):
        raise Exception("test")

    context = MagicMock()
    consumer = AIOConsumer(context, pattern="acapy-inbound-.*")
    aio_kafka = mocker.patch("kafka_queue.aio_consumer.AIOKafkaConsumer").return_value
    logger = mocker.patch("kafka_queue.aio_consumer.LOGGER")
    aio_kafka.start.side_effect = aux_function
    aio_kafka.subscribe.side_effect = aux_subscribe
