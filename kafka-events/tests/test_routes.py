...
from kafka_queue.routes import post_process_routes, register, stop_kafka, start_kafka
...
async def test_stop_kafka(web_request):
    with mock.patch("kafka_queue.routes.teardown") as teardown:
...
async def test_start_kafka(web_request):
    with mock.patch("kafka_queue.routes.start") as start:
