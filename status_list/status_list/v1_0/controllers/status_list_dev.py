...
from ..models import StatusListDef, StatusListShard, StatusListShardSchema
...
class QueryStatusListRequest(OpenAPISchema):
    ...
    shard_number = fields.Str(
        required=False,
        metadata={"description": "Status list shard number."},
    )
...
async def get_status_lists(request: web.BaseRequest):
    ...
    try:
        ...
        async with context.profile.session() as session:
            tag_filter = {
                attr: value
                for attr in ("list_number", "shard_number")
                if (value := request.query.get(attr))
            }
            tag_filter["definition_id"] = definition_id
            records = await StatusListShard.query(session, tag_filter)
...
@docs(
    tags=["status-list"],
    summary="Search status list entry by list number and entry index",
    deprecated=True,
)
...
async def get_status_list_entry(request: web.BaseRequest):
    """Request handler for querying status list entry by list number and entry index."""

    definition_id = request.match_info["def_id"]
    list_number = request.match_info["list_num"]
    entry_index = int(request.match_info["entry_idx"])
