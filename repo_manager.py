...
def main(arg_1 = None):
    ...
    # Update plugins description with supported aries-cloudagent version
        ...
        # Used for any new plugins that have been added since the last release
        all_plugins = os.listdir('./')
        # Get the last release version from the plugin_globals poetry.lock file
        with open('./plugin_globals/poetry.lock', 'r') as file:
            for line in file:
                if 'name = "aries-cloudagent"' in line:
                    next_line = next(file, None)
                    global_version = re.findall(r'"([^"]*)"', next_line)
                    break
        # Extract the latest plugin versions from the RELEASES.md file
            ...
            for line in file:
                if f'### Release v{global_version[0]}' in line:
                    ...
                    while '***' not in line:
                        ...
                        line = next(file)
                    break
        # Remove the last element which is the *** line
        if len(last_releases) != 0:
            last_releases.pop()

        # If the version matches the global version, then it wass not upgraded
        plugins_on_old_release = []
        for plugin in last_releases:
            if plugin in all_plugins: 
                all_plugins.remove(plugin)
            if (plugin.split('|')[2].strip() == global_version[0]):
                plugins_on_old_release.append(plugin.split('|')[1].strip())

        # Add any new plugins. Considered upgraded.
        plugins_on_old_release += all_plugins
