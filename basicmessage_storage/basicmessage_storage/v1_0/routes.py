...
class BasicMessageListSchema(OpenAPISchema):
    """Basic Messages List results schema."""
...
async def register(app: web.Application):
    ...
    # just in case we get loaded before the builtin send-message (should be impossible)
    # let's make sure we've added endpoint we expect
    if not has_send_message:
        LOGGER.info("adding POST /connections/<conn_id>/send-message route")
        app.add_routes(
            [
                web.post(
