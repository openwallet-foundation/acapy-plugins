...
import pytest
from rpc.v1_0.routes import DRPCRequestResultSchema, DRPCRequestSchema, DRPCResponseResultSchema, DRPCResponseSchema
...
def test_valid_drpc_request_result():
    """Test the DRPCRequesResultSchema schema."""
    ...
    schema = DRPCRequestResultSchema()
    result = schema.load(data)
...
def test_valid_drpc_response_result():
    """Test the DRPCResponseResultSchema schema."""
    ...
    schema = DRPCResponseResultSchema()
    result = schema.load(data)
    assert result._id is not None
    assert result._type is not None
    assert result.conn_id == "3fa85f64-5717-4562-b3fc-2c963f66afa6"
