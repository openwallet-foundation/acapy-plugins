...
class TestRoutes(AsyncTestCase):
    ...
    @asynctest.patch.object(WalletRecord, "retrieve_by_id")
    async def test_plugin_wallet_create_token_returns_token_with_wallet_key(
        ...
        mock_manager = async_mock.MagicMock(BaseMultitenantManager)
        mock_manager.create_auth_token.return_value = "token"
        self.context.profile.inject = async_mock.Mock(
            side_effect=[
        ...
        mock_retrieve.return_value = async_mock.CoroutineMock(
            requires_external_key=lambda: True
        )
    ...
    async def test_plugin_wallet_create_token_with_requires_external_token_and_token_raise_bad_request(
        ...
        self.request.context.profile.inject = async_mock.MagicMock(
    ...
    async def test_register_replaces_endpoints_without_adding_routes(self):
        ...
        mock_app.router.routes = async_mock.MagicMock()
