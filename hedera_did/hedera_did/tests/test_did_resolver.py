...
class TestDidResolver:
    ...
    async def test_resolve_success(self, mock_hedera_did_resolver, profile, context):
        ...
        mock_hedera_did_resolver.return_value.resolve = AsyncMock(return_value = did_document)
    ...
    @patch("hedera_did.did_resolver.SdkHederaDidResolver")
    async def test_resolve_invalid_hedera_network(self, mock_hedera_did_resolver, profile, context):
        ...
        with pytest.raises(ResolverError, match="DID string is invalid. Invalid Hedera network."):
