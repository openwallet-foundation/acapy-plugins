...
async def get_private_key_der_from_did(wallet: BaseWallet, did: str):
    ...
    key_entry = await wallet._session.handle.fetch_key(name=pubkey_base58[1:])
...
def inject_or_fail(session: ProfileSession, base_class: Type[InjectType], exception) -> InjectType:
    """Inject class from context or fail if not possible."""
