...
from acapy_agent.askar.store import AskarOpenStore, AskarStoreConfig
...
from acapy_agent.core.profile import Profile, ProfileManager, ProfileSession
...
from acapy_agent.utils.multi_ledger import get_write_ledger_config_for_profile
...
class AskarProfile(Profile):
    ...
    def bind_providers(self):
        ...
        injector.bind_provider(
            IndyHolder,
            ClassProvider(
                "indy_ledger.indy.credx.holder.IndyCredxHolder",
                ref(self),
            ),
        )
        injector.bind_provider(
            IndyIssuer,
            ClassProvider("indy_ledger.indy.credx.issuer.IndyCredxIssuer", ref(self)),
        )
        ...
        if self.ledger_pool or self.settings.get("ledger.ledger_config_list"):
            injector.bind_provider(
                IndyVerifier,
                ClassProvider(
                    "indy_ledger.indy.credx.verifier.IndyCredxVerifier",
                    ref(self),
                ),
            )
...
class AskarProfileSession(ProfileSession):
    ...
    async def _setup(self):
        ...
        injector.bind_provider(
            BaseWallet,
            ClassProvider("indy_ledger.wallet.askar.IndyAskarWallet", ref(self)),
        )
        injector.bind_provider(
            BaseStorage,
