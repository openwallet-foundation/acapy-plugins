...
from acapy_agent.utils.env import storage_path
from acapy_agent.utils.general import strip_did_prefix
from acapy_agent.wallet.base import BaseWallet, DIDInfo
from acapy_agent.wallet.did_posture import DIDPosture
from acapy_agent.wallet.error import WalletNotFoundError
from indy_vdr import Pool, Request, VdrError, ledger, open_pool

from ..indy.valid import IndyDID
...
class IndyVdrLedger(BaseLedger):
    ...
    async def send_revoc_reg_def(
        ...
        if self.profile.context.settings.get("wallet.type") in (
            ...
            from indy_ledger.anoncreds.registry import (
                LegacyIndyRegistry,
            )
